...
def render_client(client_id: str, client_name: str, coach_mode: bool = False):
    ...
    with tab1:
        ...
        fig_w = go.Figure()
        fig_w.add_trace(go.Scatter(x=df['date'], y=df['weight'] * KG_TO_LB, name='Daily', mode='lines',
                                   line=dict(color='lightblue', width=1), opacity=0.5))
        fig_w.add_trace(go.Scatter(x=df['date'], y=df['weight_avg'] * KG_TO_LB, name='14-Day Avg', mode='lines',
    ...
    with tab4:
        ...
        c1.metric("Avg Weight (14d)", f"{latest.get('weight_avg', 0) * KG_TO_LB:.2f} lb",
        ...
        rec_df['weight'] = (rec_df['weight'] * KG_TO_LB).round(2)
        for col in ['calories_avg', 'steps_avg']:
    ...
    with tab6:
        ...
        for _, row in periods.iterrows():
            with st.expander(f"**{row['period_label']}** &nbsp; {row['Date Range']}", expanded=False):
                ...
                c1.metric("Avg Weight", f"{row['Avg_Weight'] * KG_TO_LB:.2f} lb",
        ...
        pw_df = df[['date', 'weight_avg', 'period_label']].copy()
        pw_df['weight_avg_lb'] = pw_df['weight_avg'] * KG_TO_LB
        fig_pw = px.line(pw_df, x='date', y='weight_avg_lb', color='period_label',
                         title="Weight Trend Across Periods",
                         labels={'weight_avg_lb': 'Avg Weight (lb)', 'date': 'Date', 'period_label': 'Period'})
